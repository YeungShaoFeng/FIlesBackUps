...
class Small:
    ...
    def update(self):
        if (self.password):
            self.password_hash_value = Small.hasher(self, self.password)
            self.password_hash_value_sum = Small.sum(self, list(self.password_hash_value))
        else:
    ...
    def smallin(self, printMode):
        Small.update(self)
        self.string = Small.shift(self, self.string, self.password_hash_value, self.password_hash_value_sum, self.uni)
        Gone = Small.stringPrint(self, 'cipher', self.string) if (printMode) else 1+1
        return None

    # 解密
    def smallout(self, printMode, fileMode):
        Small.update(self)
        self.string = Small.unshift(self, self.string, self.password_hash_value, self.password_hash_value_sum, self.uni, fileMode)
